...
function extractGeminiText(response: { text?: string; candidates?: Array<{ content?: { parts?: Array<{ text?: string }> } }> }): string {
  try {
    ...
    // Getter can throw on unexpected candidate shapes.
  }
  try {
    const parts = response?.candidates?.[0]?.content?.parts;
    if (Array.isArray(parts)) {
      const joined = parts.map((part) => (typeof part.text === "string" ? part.text : "")).join("").trim();
      if (joined) return joined;
    }
  } catch {
    // Ignore malformed SDK payloads.
  }
  return "";
}

async function generateGeminiText(prompt: string, model: string): Promise<string> {
  const ai = getAiClient();
  if (!ai) return "";

  const modelsToTry = [...new Set([model, "gemini-2.5-flash", "gemini-2.0-flash"].filter(Boolean))];
  ...
  for (const candidateModel of modelsToTry) {
    try {
      ...
      lastError = err;
      console.warn(`[Gemini] ${candidateModel} failed:`, err instanceof Error ? err.message : err);
    }
  }
...
Respond with ONLY valid JSON:
{
  ...
  app.post("/api/assistant/chat", async (req, res) => {
    try {
      ...
      const fallbackReply = buildAssistantFallbackReply(message, normProf);

      // Rank the full compact KB by question + profile so CGTMSE / 43B(h) / RoDTEP
      // are not dropped by an arbitrary first-N slice.
      const relevantSchemes = selectSchemesForAssistant(message, normProf);
...
4. Prefer citing the official_source and required_documents from the knowledge base when answering "what documents do I need" questions.

User Business Profile:
${normProf ? JSON.stringify(normProf) : "Not provided"}

MSME Scheme Knowledge Base Context (Verified Government Schemes, ranked for this question):
...
Respond in clear, concise, practical natural language for an Indian MSME owner. Use bullet points where appropriate.`;
      ...
      try {
        const replyText = await generateGeminiText(prompt, GEMINI_MODEL);
        if (replyText) {
          res.json({ response: replyText });
          return;
        }
      } catch (llmErr: any) {
        console.warn("Gemini assistant failed, using knowledge-base reply:", llmErr?.message || llmErr);
      }

      res.json({ response: fallbackReply, via: "knowledge_base" });
    } catch (err: any) {
      ...
      try {
        ...
        res.json({ response: fallback, via: "knowledge_base" });
      } catch {
        res.status(500).json({
  ...
  // Files stay in RAM via multer.memoryStorage() and are discarded when the
  // request ends — never written to disk.
  app.post("/api/documents/extract", (req, res, next) => {
    upload.single("document")(req, res, (err: any) => {
      if (err instanceof multer.MulterError) {
        ...
        res.status(400).json({ error: "Invalid file upload. Please try a PDF or JPG/PNG under 5MB." });
        return;
      }
      if (err) {
        res.status(400).json({ error: "Couldn't read this document, please enter details manually." });
        return;
      }
      next();
    });
  ...
  "documentType": "udyam_certificate | gst_certificate | unknown"
      ...
      for (const [key, value] of Object.entries(extractedData || {})) {
        if (typeof value === "string") {
          ...
          normalized[key] = trimmed.length === 0 || trimmed.toLowerCase() === "null" ? null : trimmed;
        } else {
          normalized[key] = (value as string | null) ?? null;
        }
      }

      const hasUsableField = Boolean(
        normalized.udyamNumber ||
          normalized.gstNumber ||
          normalized.businessName ||
          normalized.enterpriseSize ||
          normalized.state
      );
      if (!hasUsableField) {
        res.status(422).json({
      ...
      // file.buffer is GC'd after this response — nothing is persisted.
      res.json({ success: true, extractedData: normalized });
    } catch (err: any) {
